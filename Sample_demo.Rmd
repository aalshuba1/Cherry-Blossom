...
```{r}
#| eval: true
NOAA_WEB_API_TOKEN <- 'pHGyMKtJUWWcVVZkAyVDTaXknaQNJqfw'
# or
...
get_daily_avg_temp <- function(station_id, start_date, end_date,
  ...
  batches <- purrr::compact(batches)
  
  map(batches, \(x) nested_to_tibble(x$results)) |> 
    list_rbind() |> 
    mutate(date = as.Date(substr(as.character(unlist(date)), 1, 10)))
}
```
...
historic_temperatures <- cherry |>
  group_by(location) |>
  summarize(start_date = sprintf('%d-01-01', max(1970, min(year) - 1))) |>
...
# remove NYC because not enough data
cherry_no_nyc <- cherry |> 
  filter(location != "newyorkcity")

# Linear regression with interaction (separate slope for each site)
ls_fit <- lm(bloom_doy ~ 0 + location + location:year,
             data = cherry_no_nyc,
             subset = year >= 1880)
...
summary(ls_fit_for_van)
```

```{r}
vancouver_grid <- tibble(
  location = "vancouver",
  year = 2023:2025
)

predictions_vancouver <- vancouver_grid |>
  bind_cols(predict(ls_fit_for_van,
...
ls_fit_for_nyc <- lm(
  bloom_doy ~ year,
  data = cherry_with_nyc,
  subset = year >= 1880,
  weights = (location == "newyorkcity") +
            0.2 * (location != "newyorkcity")
)

summary(ls_fit_for_nyc)

```

```{r}
df <- cherry |> inner_join(avg_winter_temp, by = c("location", "year"))

ls_fit_with_temp <- lm(bloom_doy ~ year * location + avg_tmax, data = df)

summary(ls_fit_with_temp)
```
...
cherry_grid <- expand_grid(
  ...
  inner_join(avg_winter_temp, by = c("location", "year"))

predictions_from_temp <- cherry_grid |>
  mutate(pred_bloom = predict(ls_fit_with_temp,
                              newdata = cherry_grid))
...
```{r}
model <- lm(bloom_doy ~ year * location, data = cherry)
summary(model)
...
```{r}
final_model <- lm(bloom_doy ~ year * location, data = cherry)

summary(final_model)
```

```{r}
future_2026 <- data.frame(
  location = unique(cherry$location),
  year = 2026
)
...
```{r}
predictions_2026 <- predict(final_model, newdata = future_2026)

future_2026$predicted_doy <- round(predictions_2026)

```


```{r}
future_2026$predicted_date <- as.Date(
  paste(future_2026$year, future_2026$predicted_doy),
  format = "%Y %j"
)
